...
# load dataset
# fields = ['date','no2']
# series = read_csv('data2.csv', header=0, parse_dates=[1], index_col=1, usecols=fields, squeeze=True, date_parser=parser)
...
delhi_data_no2['date'] = delhi_data_no2['date'].map(lambda x: str(x)[:7] + '-01')
delhi_data_no2['date'] = delhi_data_no2['date'].map(lambda x: parser(x))

delhi_data_no2['date'] = pd.DatetimeIndex(delhi_data_no2['date'])

series = delhi_data_no2.groupby('date').no2.mean()
series.plot()
pyplot.show()
